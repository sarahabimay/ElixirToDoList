defmodule ToDoListAppTest do
  ...
  test "it maps '9' to nothing" do
    ...
    capture_io([input: user_input], fn ->
      assert ToDo.ToDoListApp.option_to_command(option, []) == :ok
    end)
  end
  ...
  test "it maps 'a' menu option to ':add' command" do
    option = "a"
    capture_io(fn ->
      assert ToDo.ToDoListApp.option_to_command(option, []) == :add
    end)
  end
  ...
  test "it maps 'b' menu option to ':list_tasks' command" do
    option = "b"
    capture_io(fn ->
      assert ToDo.ToDoListApp.option_to_command(option, []) == :list_tasks
    end)
  end
  ...
  test "it maps 'c' menu option to ':edit' command" do
    option = "c 1"
    capture_io(fn ->
      assert ToDo.ToDoListApp.option_to_command(option, []) == {:edit, "1"}
    end)
  end
  ...
  test "it maps 'd' menu option to ':delete' command" do
    option = "d 1"
    capture_io(fn ->
      assert ToDo.ToDoListApp.option_to_command(option, []) == {:delete, "1"}
    end)
  end
  ...
  test "it maps 'e' menu option to ':exit' command" do
    exit = "e"
    capture_io(fn ->
      assert ToDo.ToDoListApp.option_to_command(exit, []) == :exit
    end)
  end
  ...
  test "it can add new tasks with the add command" do
    ...
    capture_io([input: user_input, capture_prompt: false], fn ->
      assert ToDo.ToDoListApp.get_new_tasks([], :add) == expected_result
    end)
  end
  ...
  test "it can list all to do tasks with the list_tasks command", context do
    ...
    capture_io([input: user_input, capture_prompt: false], fn ->
      assert ToDo.ToDoListApp.process_command(:list_tasks, context[:tasks]) == expected_result
    end)
  end
  ...
  test "it can edit a task with the edit command", context do
    ...
    capture_io([input: user_input], fn ->
      assert ToDo.ToDoListApp.process_command({:edit, task_number}, context[:tasks]) == expected_result
    end)
  end
  ...
  test "it can delete a task with the delete command", context do
    ...
    assert ToDo.ToDoListApp.process_command({:delete, task_number}, context[:tasks]) == expected_result
  end

  test "user enters two tasks then quits" do
    user_input = "a\nFinish Todo List\nWrite blog\n\ne"
    capture_io([input: user_input, capture_prompt: false], fn ->
      assert ToDo.ToDoListApp.todo_list_run([]) == :ok
    end)
  end
end
